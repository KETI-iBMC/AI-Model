...
void* waiting_death(void* data) {
    ...
    if (-1 == (msqid_req = msgget((key_t)1111, IPC_CREAT|0666))) { // 요청 큐 전체의 id를 받아옴(대문). 한번만 함
    ...
    if (-1 == msgrcv(msqid_req, &msq_req, sizeof(kvm_msq_t)-sizeof(long), 0, 0)) { // BLOCKED
    ...
    if (_waiting_death() == 0) {
        msq_rsp.type = 1;
        msq_rsp.ccode = 0;
        if ( -1 == msgsnd(msqid_rsp, &msq_rsp, sizeof(kvm_msq_t)-sizeof(long), 0))
        {
